...
function App() {
  ...
  const removeAudioChannel = async (e) => {
    ...
    ffmpeg.FS("writeFile", "image.png", await fetchFile(droppedFile.payload));

    //await ffmpeg.run("-framerate", "1/10", "-i", "noAudio.mp4", "-c:v", "libx264", "-pix_fmt", "yuv420p", "-vf", "scale=1920:1080", "test.mp4");
    await ffmpeg.run("-framerate", "1/10", "-i", "image.png", "-c:v", "libx264", "-t", "10", "-pix_fmt", "yuv420p", "-vf", "scale=1920:1080", "test.mp4");
